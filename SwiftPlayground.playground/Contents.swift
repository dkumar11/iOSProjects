...
/// Helper function that takes in a word and checks if the word if a palindrome
/// Main Idea:
/*I teratates through each element by a pointer from the front and a pointer from
  the back and check if the characters are the same. If they are the same the pointers
  are updated, if they are different than the word is not a palindrome*/

func isPalindrome(_ word: String) -> Bool {
    let wordArr = Array(word.lowercased())
    var i = 0
    var j = word.count - 1
    while i != j && i < j{
...
isPalindrome("abbaa")
isPalindrome("abbbba")
arePalindromes(["peep","Heelopoleeh","IrirI","RacerecaR","Piperepip"])
...
func findStudent(withStudentID sid: Int?, studentIDNumbers: [Int: String] = getSIDsDictionary()) -> String? {
    ...
    if sid != nil {
        let value = studentIDNumbers[sid!]
        if value != nil {
            return value! + " stole the cookie!"
        }
    }
    return nil
}
...
//print("The unknown buildings capacity is", building.getCapacity())
//print("Soda Hall's capacity is", sodaHall.getCapacity())
//print("Cory Hall's capacity is",coryHall.getCapacity())
